...
export default class MyPerformance extends Component {
  ...
  renderItem = ({item, index}) => {
    return (
      <TouchableOpacity
        onPress={() => {
          if (this.state.ShowSubjectIndex == index) {
            this.setState({ShowSubjectIndex: -1});
          } else {
            this.setState({ShowSubjectIndex: index});
            this.onLoadGetSubjectWiseBlockBox(
              item.batchYear,
              item.degreeCode,
              item.semester,
        ...
        style={{
          ...
          borderColor: '#5182E9',
          backgroundColor:
            this.state.ShowSubjectIndex == index ? '#E1EFF9' : 'transparent',
        }}>
        <Text
          style={{
        ...
        <View
          ...
          this.state.ShowSubjectIndex == index ? (
        ...
        {this.state.ShowSubjectIndex == index ? (
          !this.state.Loader2 && this.state.ShowSubjectIndex == index ? (
  ...
  renderMapFunction = ({item, index}) => {
    ...
    //);
    data2.push(
      {data: range40.length == 0 ? 1 : range40.length, color: '#007AFF'},
      {data: range50.length == 0 ? 1 : range50.length, color: '#CB5B66'},
      {data: range65.length == 0 ? 1 : range65.length, color: '#627281'},
      {data: range75.length == 0 ? 1 : range75.length, color: '#F39800'},
      {data: range90.length == 0 ? 1 : range90.length, color: '#B033AB'},
      {
        data: range100.length == 0 ? 1 : range100.length,
        color: '#17B6FF',
      },
      {
        data: NA.length == 0 ? 1 : NA.length,
        color: 'red',
      },
    );
    console.log(data2);
          ...
          <View
            ...
            <View style={styles.numberRangeView}>
              ...
              <View
                ...
                <CustomProgress
                  progress={NA.length / this.state.StudentCount}
